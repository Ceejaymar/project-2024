...
type MosaicThemeStyle = React.CSSProperties & {
  '--mosaic-gold': string;
  '--mosaic-gold-text': string;
  '--mosaic-bg': string;
  '--mosaic-text': string;
  '--mosaic-muted': string;
...
function useMosaicThemeStyle(): MosaicThemeStyle {
  ...
  return {
    '--mosaic-gold': MOSAIC_GOLD,
    '--mosaic-gold-text': getCaseStudyGoldTextColor(
      theme.colors['default-text'],
    ),
    '--mosaic-bg': theme.colors.background,
    '--mosaic-text': theme.colors['default-text'],
    '--mosaic-muted': theme.colors['secondary-text'],
...
const TYPE_ROLE_ROWS = [
  ...
  },
  {
    ...
    description:
      'Monospace labels for dates, metadata, and small system details.',
  },
] as const satisfies ReadonlyArray<{
...
const ARCHITECTURE_COLUMNS = [
  ...
  ArchitectureDataColumn | ArchitectureReasonColumn
>;
  ...
  const cells: Array<HeroCalendarDay | null> = Array.from(
    ...
    (_, cellIndex): HeroCalendarDay | null => {
      ...
      const startingFamilyIndex =
        (day * 2 + monthIndex) % MOSAIC_EMOTION_FAMILIES.length;
      const colors = Array.from({ length: segmentCount }, (_, colorOffset) => {
        const familyIndex =
          (startingFamilyIndex + colorOffset) % MOSAIC_EMOTION_FAMILIES.length;
...
function MosaicHeroScene() {
  const mosaicThemeStyle = useMosaicThemeStyle();

  return (
    <figure
      className={styles.heroScene}
      style={mosaicThemeStyle}
      aria-labelledby="mosaic-hero-title"
    >
          ...
          <div className={styles.phoneScreen}>
            <div className={styles.phoneTop}>
              ...
              <span className={styles.phoneBadge}>
                {heroCalendar.daysInMonth} day streak
              </span>
            </div>
...
export default function MosaicCaseStudy() {
  ...
  return (
    ...
    >
      <div className={styles.mosaicScope} style={mosaicThemeStyle}>
        <CaseStudySection title="Overview">
          <p>
            Mosaic is an emotion journal built for quick, low-pressure
            reflection. People start with one of seven emotion families, add
            detail only when it helps, and can save a check-in in as few as
            three taps. Up to four check-ins become one daily mosaic tile,
            creating a visual record that makes shifts over time easier to
            revisit.
          </p>
        </CaseStudySection>
        ...
        <CaseStudySection title="Competitive review signals" spacing="spacious">
          <p className={styles.researchMethod}>
            To understand where existing mood trackers break down, I reviewed
            150+ public App Store reviews across 5+ mood-tracking and journaling
            apps. I grouped repeated feedback into three themes that shaped
            Mosaic’s interaction model.
          </p>
          ...
          <p className={styles.researchInsightNote}>
            Reviews also showed that logging alone was not enough. People wanted
            a clearer way to look back at emotional patterns without being
            pushed into a dense dashboard. That informed Mosaic’s weekly and
            monthly insight views.
          </p>
        </CaseStudySection>
          ...
          >
            <article className={styles.interactionDecision}>
              ...
              <p className={styles.interactionDecisionText}>
                People do not always need the same level of emotional
                specificity. Mosaic begins with seven emotion families, then
                lets someone explore more specific feelings only when they want
                to. The default stays quick, without treating every day as
                simple.
              </p>
              <div
                className={styles.interactionFlowVisual}
                role="group"
                aria-label="Check-in flow screenshots"
              >
                {CHECK_IN_SCREENSHOTS.map((screenshot) => (
                  <figure
                    className={styles.interactionScreenImageFrame}
                    key={screenshot.alt}
                  >
                    <img
              ...
              <p className={styles.interactionFlowCaption}>
                Every check-in starts with a broad choice. More specific
                feelings, notes, and context tags remain available without
                becoming required steps.
              </p>
            </article>

            <article className={styles.interactionDecision}>
              <p className={styles.interactionDecisionEyebrow}>
                Represent change
              </p>
              <h3 className={styles.interactionDecisionTitle}>
              ...
              <p className={styles.interactionDecisionText}>
                ...
                up to four check-ins, then combines them into one daily tile.
                That makes emotional shifts visible without turning a day into a
              ...
              <div>
                ...
                <p className={styles.tileProgressionCaption}>
                  Each new check-in adds another segment, so one tile can hold
                  the shape of a changing day.
                </p>
              </div>
            </article>
            ...
            <article className={styles.interactionDecision}>
              ...
              <p className={styles.interactionFlowCaption}>
                ...
                makes the longer rhythm of emotional reflection easier to see at
                a glance.
              </p>
            </article>
          </div>
        ...
        <CaseStudySection
          title="Designing the visual system"
          spacing="spacious"
        >
          <p className={styles.visualSystemLead}>
            Mosaic uses a dark, quiet foundation so emotional reflection can
            feel personal rather than clinical. The interface stays restrained
            while color, type, and small moments of motion give each check-in
            its own presence.
          </p>
          ...
          <div className={styles.visualFoundationsPanel}>
            <div className={styles.visualFoundationColumn}>
              <div className={styles.foundationGroup}>
                ...
                <div
                  className={styles.colorTokenGrid}
                  aria-label="Mosaic system palette"
                >
                  {SYSTEM_PALETTE_TOKENS.map((token) => (
                    <div className={styles.colorToken} key={token.name}>
                      ...
                      <span>
                        ...
                        <span className={styles.colorTokenValue}>
                          {token.color}
                        </span>
                      </span>
                    </div>
                    ...
                    return (
                      <div className={styles.emotionColorItem} key={emotion.id}>
                        ...
                        <span>
                          ...
                          <span className={styles.emotionColorValue}>
                            {color}
                          </span>
                        </span>
                      </div>
            ...
            <div className={styles.visualFoundationColumn}>
              <div className={styles.foundationGroup}>
                ...
                <h4 className={styles.foundationTitle}>
                  Editorial warmth, clear utility
                </h4>
                <p className={styles.foundationText}>
          ...
          <section className={styles.visualSubsection}>
            <h3 className={styles.visualSubheading}>
              Quiet interface, expressive emotion
            </h3>
            <div className={styles.visualFeatureLayout}>
              <div>
                <p>
                  Mosaic keeps the interface deliberately quiet. The black
                ...
                <p>
                  ...
                  Emotion colors carry the personal signal, while the
                  surrounding interface makes space for the moment instead of
          ...
          <section className={styles.visualSubsection}>
            ...
            <div className={styles.accessibilityLayout}>
              <div>
                <p>
                  Accessibility is part of the same product system, not a
                  separate version of Mosaic. High-contrast text makes subtle
                  text and borders easier to see. Reduced motion removes screen
                  transitions and heavier animations. Haptic feedback can also
                  be turned off entirely.
                </p>
                <p>
                  Those settings let the interface adapt to different needs
        ...
        <CaseStudySection title="Architecture" spacing="spacious">
          <div className={styles.architectureContent}>
            ...
            <p className={styles.architectureLead}>
              The technical work focused on making a simple check-in useful
              beyond the moment it is saved. A single entry can shape the day’s
              mosaic and contribute to longer-term patterns, while Mosaic keeps
              the emotional language consistent across the app.
            </p>
            ...
            <div className={styles.architecturePanel}>
              ...
              <div className={styles.architecturePanelGrid}>
                {ARCHITECTURE_COLUMNS.map((column) => (
                  <div
                    className={styles.architecturePanelColumn}
                    key={column.eyebrow}
                  >
                    <p className={styles.architectureEyebrow}>
                      {column.eyebrow}
                    </p>
                    <h4 className={styles.architectureColumnTitle}>
                      {column.title}
                    </h4>
                    <p className={styles.architectureColumnText}>
                      {column.text}
                    </p>
                    {column.kind === 'data' ? (
                      <div
                        className={styles.architectureDataList}
                        aria-label={column.ariaLabel}
                      >
                        {column.items.map((item) => (
                          <div
                            className={styles.architectureDataItem}
                            key={item.label}
                          >
                            <span className={styles.architectureDataLabel}>
                              {item.label}
                            </span>
                            <span className={styles.architectureDataValue}>
                              {item.value}
                            </span>
                          </div>
                        ))}
                      ...
                      <ul className={styles.architectureReasonList}>
                        {column.reasons.map((reason) => (
                          <li
                            className={styles.architectureReasonItem}
                            key={reason}
                          >
                            {reason}
                          </li>
            ...
            <p className={styles.architecturePostCopy}>
              ...
              looks up those display details when it needs them. That leaves
              room to refine wording, add translations, or adjust palettes over
              time without rewriting someone’s history.
            </p>

            <CaseStudyCallout label="Key takeaway">
              One small check-in can stay useful across the mosaic, insights,
              and future product improvements without changing someone’s
              history.
            </CaseStudyCallout>
          </div>
        </CaseStudySection>
        ...
        >
          <p>
            Mosaic is live and still early. The next phase is focused on
          ...
          <div className={`${styles.dividedList} ${styles.nextDirectionsList}`}>
            <article className={styles.interactionDecision}>
              ...
              <p className={styles.interactionDecisionText}>
                The current four-entry limit keeps a day easy to read at a
                glance. I&apos;ll use privacy-conscious interaction signals to
                understand whether people reach that limit or want to capture
                more moments. A higher limit is worth testing only if the daily
                mosaic can still make those moments easy to read.
              </p>
            </article>
            ...
            <article className={styles.interactionDecision}>
              ...
              <p className={styles.interactionDecisionText}>
                Explore alternative emotion palettes and supporting visual cues
                so Mosaic&apos;s patterns remain easy to distinguish across
                different forms of color vision. The goal is to preserve color
                as a meaningful part of reflection while making the system
                easier for more people to read.
              </p>
            </article>
          </div>
          <CaseStudyCallout label="Key takeaway">
